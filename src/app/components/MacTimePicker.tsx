...
function Column<T extends string | number>({
  ...
  return (
    <div className="flex flex-col overflow-y-auto max-h-[220px] scrollbar-none px-1 py-1">
      {items.map((item) => {
        ...
        return (
          <button
            ...
            className={clsx(
              "px-3 py-[5px] text-[13px] rounded-[7px] transition-all text-left whitespace-nowrap select-none",
              isSelected
                ? "bg-[#0A84FF] text-white font-semibold"
                : "text-foreground/55 dark:text-[#EBEBF5]/55 hover:text-foreground/90 hover:bg-black/5 dark:hover:text-[#EBEBF5]/90 dark:hover:bg-white/[0.07]"
            )}
          >
            {label(item)}
          </button>
  ...
  return (
    <Popover.Root>
      <Popover.Trigger asChild>
        ...
        >
          <Clock className="w-3 h-3 text-foreground/35 dark:text-white/35 shrink-0" />
      ...
      <Popover.Portal>
        ...
        >
          ...
          {/* Thin divider */}
          <div className="w-px bg-border my-2 self-stretch shrink-0" />
